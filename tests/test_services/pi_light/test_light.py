...
class TestLight(TestCase):
    ...
    def test_add_multiple_rules_to_day(self) -> None:
        ...
        self.light.add_rule(rule1)
        self.light.add_rule(rule2)
    ...
    def test_rules_are_sorted(self) -> None:
        ...
        self.light.add_rule(rule2)
        self.light.add_rule(rule4)
        self.light.add_rule(rule1)
        self.light.add_rule(rule3)
    ...
    def test_add_conflicting_rules_to_day(self) -> None:
        ...
        self.light.add_rule(rule3)
        self.light.add_rule(rule4)
        self.light.add_rule(rule5)
        self.light.add_rule(rule6)
        self.light.add_rule(rule7)
        self.light.add_rule(rule8)
        self.light.add_rule(rule9)
    ...
    def test_add_invalid_rule_invalid_start_time(self) -> None:
        with pytest.raises(ValidationError):
            self.light.add_rule(Rule(start_time=30, stop_time=8))

    def test_add_invalid_rule_bad_start_time(self) -> None:
        with pytest.raises(ValidationError):
            self.light.add_rule(Rule(start_time=-1, stop_time=8))

    def test_add_invalid_rule_bad_stop_time(self) -> None:
        with pytest.raises(ValidationError):
            self.light.add_rule(Rule(start_time=20, stop_time=-1))

    def test_add_invalid_rule_bad_color(self) -> None:
        with pytest.raises(ValidationError):
            self.light.add_rule(Rule(start_color=Color(r=-1)))
